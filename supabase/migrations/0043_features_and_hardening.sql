-- =============================================================================
-- 0043_features_and_hardening.sql
-- 1. app_is_platform_admin(uuid) overload — used by the Drive edge functions
--    (service-role RPC probe) and the 0042 RLS policy, but never previously
--    defined. Without it every admin-gated Drive call fails closed.
-- 2. formula_library — shared formula reference (spec §52).
-- 3. question_translations — multilingual architecture (spec §51).
-- 4. RLS for solutions + question_usage (were intentionally open in 0038;
--    tightened now that the UI writes through RPCs / staff policies).
-- 5. omr_sheets.scan_config — bubble geometry captured at render time so the
--    client-side OMR detector can locate bubbles on scanned sheets.
-- 6. app_parent_dashboard() — server-authorized ward data for PARENT role.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Platform-admin probe with an explicit user id (edge functions run with
...
-- -----------------------------------------------------------------------------
-- 3. Question translations (multilingual architecture, spec §51)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. RLS for solutions + question_usage (previously open to all writes)
-- -----------------------------------------------------------------------------
...
create policy question_usage_write on question_usage
  ...
  with check (tenant_id is not null and app_can_access_tenant(tenant_id)
         and (app_is_platform_admin() or not app_user_has_student_only_role(tenant_id)));
-- Engine RPCs (app_generate_paper / dpp / manual) are SECURITY DEFINER and
-- therefore unaffected by these policies.

-- -----------------------------------------------------------------------------
-- 5. OMR scan geometry (captured when the printable sheet is rendered)
-- -----------------------------------------------------------------------------
do $$
begin
...
-- -----------------------------------------------------------------------------
-- 6. Parent dashboard RPC — server-authorized ward access.
